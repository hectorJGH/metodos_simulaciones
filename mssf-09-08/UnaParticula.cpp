...
//---- declarar constantes ---
const double g=9.8, K=1.0e4, Gamma=1000, Kcundall=10, MU=0.4;
...
void Cuerpo::Mueva_V(double dt, double Coeficiente){
  V+=F*(Coeficiente*dt/m); omega+=tau*(Coeficiente*dt);
}
...
void Colisionador::CalculeFuerzaEntre(Cuerpo & Grano1, Cuerpo & Grano2){
  ...
  double m12=Grano1.m*Grano1.m/(Grano1.m+Grano2.m);
  vector3D V21= Grano2.V - Grano1.V;
  if(s>0){
    vector3D n=r21*(1.0/d);
    vector3D F2=n*(K*pow(s,1.5))-(Gamma*sqrt(s)*m12)*V21;
...
int main(void){
  ...
  int i,ix,iy;
  double t,tdibujo,tmax=10*sqrt(Ly/2*g),tcuadro=tmax/1000,dt=1e-3;
  ...
  for(ix=0;ix<Nx;ix++)
    for(iy=0;iy<Ny;iy++){
      ...
      //--------------------(   x0,   y0,          Vx0,   Vy0,theta0,omega0 m0,R0,)
      Grano[Nx*iy+ix].Inicie((ix+1)*dx,(iy+1)*dy,      0.0,  0,0, omegamax*ran64.r(), m0,R0);//OJO
    }
